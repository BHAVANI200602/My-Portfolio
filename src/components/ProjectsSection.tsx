...
export default function ProjectsSection() {
  ...
  return (
    <section id="section-4" className="w-full bg-[#FF3744]">
      {/* Zoom Phase Container */}
      <div ref={containerRef} className="relative h-[150vh] w-full">
        <motion.div
          className="sticky top-0 h-screen w-full overflow-hidden flex flex-col justify-center items-center px-4 md:px-8 z-10 bg-[#FF3744]"
        >
      ...
      <div className="relative z-20 w-full bg-[#FF3744]">
          <div className="w-full max-w-7xl mx-auto px-6 md:px-12 py-32 flex flex-col gap-32">
            {PROJECTS.map((project, idx) => (
              <div key={project.id} className="flex flex-col md:flex-row w-full gap-8 md:gap-16 items-start border-t border-[#030014]/20 pt-12">
                {/* Left Side: Massive Project Name */}
                <div className="w-full md:w-[45%] flex flex-col">
                  <span className="font-mono text-[10px] tracking-[0.3em] text-[#030014]/50 mb-4 uppercase">
                    {String(idx + 1).padStart(2, "0")} — {String(PROJECTS.length).padStart(2, "0")}
                  </span>
                  <h2 className="font-anton uppercase leading-[0.85] tracking-tight text-[#030014] text-6xl md:text-7xl lg:text-[7rem]">
                    {project.title}
                  </h2>
                  <p className="font-mono text-[10px] tracking-[0.2em] text-[#030014]/50 uppercase mt-4">
                    {project.subtitle}
                  </p>
                  <span className="inline-block mt-8 font-mono text-[10px] tracking-widest uppercase text-[#030014]/50 border border-[#030014]/20 px-3 py-1.5 rounded-sm w-fit">
                    {project.year}
                  </span>
                </div>
                ...
                <div className="w-full md:w-[55%] flex flex-col justify-center">
                  <p className="font-sans font-light text-sm md:text-base text-[#030014]/70 leading-relaxed mb-8 max-w-lg">
                    <TextReveal delay={0.2}>{project.description}</TextReveal>
                  </p>

                  {project.bulletPoints && project.bulletPoints.length > 0 && (
                    <div className="mb-8">
                      <span className="font-mono text-[10px] tracking-[0.3em] text-[#030014] uppercase mb-4 block">
                        Highlights
                      </span>
                      <ul className="space-y-3">
                        {project.bulletPoints.map((pt, i) => (
                          <li key={i} className="flex items-start gap-3 text-[15px] text-[#030014] font-sans font-medium leading-snug">
                            <span className="text-[#030014] mt-0.5 shrink-0 leading-none font-bold">›</span>
                            {pt}
                          </li>
                        ))}
                      </ul>
                  ...
                  <div className="flex flex-wrap gap-2 mb-10">
                    {project.tags.map((tag) => (
                      <span key={tag} className="font-mono text-[10px] tracking-widest uppercase px-3 py-1.5 border border-[#030014]/30 text-[#030014] font-semibold rounded-sm">
                        {tag}
                      </span>
                    ))}
                  </div>
