...
// Simplex-Perlin Noise via JS (from original Waves snippet)
class Grad {
...
export default function Shader7Waves() {
  const containerRef = useRef<HTMLDivElement>(null);
  const canvasRef    = useRef<HTMLCanvasElement>(null);
  const noiseRef     = useRef(new Noise(Math.random()));
  const linesRef     = useRef<Point[][]>([]);
  const mouseRef     = useRef<Mouse>({ x:-10,y:0,lx:0,ly:0,sx:0,sy:0,v:0,vs:0,a:0,set:false });
  const boundRef     = useRef({ width:0, height:0, left:0, top:0 });
  const frameRef     = useRef<number | null>(null);

  // Charcoal theme config
  const lineColor   = 'rgba(200,200,200,0.18)';
  const xGap = 14, yGap = 40;
  const waveSpeedX = 0.012, waveSpeedY = 0.005;
  const waveAmpX = 36, waveAmpY = 18;
  const friction = 0.925, tension = 0.005, maxCursorMove = 120;
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      boundRef.current = { width: rect.width, height: rect.height, left: rect.left, top: rect.top };
    ...
    const moved = (p: Point, wc=true) => ({
      x: Math.round((p.x+p.wave.x+(wc?p.cursor.x:0))*10)/10,
      y: Math.round((p.y+p.wave.y+(wc?p.cursor.y:0))*10)/10,
    });
    const drawLines = () => {
      const { width, height } = boundRef.current;
    ...
    const tick = (t: number) => {
      ...
      mouse.vs+=( mouse.v-mouse.vs)*0.1; mouse.vs=Math.min(100,mouse.vs);
    ...
    const onMouse  = (e: MouseEvent) => {
      const b=boundRef.current, m=mouseRef.current;
  ...
  return (
    <div
      ref={containerRef}
      style={{ position:'absolute',inset:0,width:'100%',height:'100%',overflow:'hidden',backgroundColor:'transparent' }}
    >
